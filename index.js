...
const commands = ["help", "new", "get", "remove", "open"];

// keylink open google => open "https://wwww.google.com"

const openLink = userInput => {
  const links = db.get("web_links").value();

  links.forEach(link => {
    if (link.name == userInput) {
      open(link.link);
    }
  });
};
...
const addName = urlName => {
  //this function saves name and link to file
  let name = chalk.blue(
    "please give your link a name example github for github.com\n"
  );
...
const getLink = () => {
  ...
  const urlList = db.get("web_links").value();
  let index = 1;
  urlList.forEach(link => {
    //loops over links in list and prints them to user with index
    let urltext = `${index++}. ${link.name}: ${link.link}`;
    console.log(chalk.green(urltext));
  });
};

const remove = () => {
  // This will remove the link the user saved
  let question = chalk.blue(
    "What is the name of the link you want to remove?\n"
  );
...
const help = () => {
  ...
  const usageText = `keylink is a tools that lets you save links as short links in
    ...
    open:     used to open the website in the browser 
              with the name of you link
